...
public class customer {
    ...
    public static void main(String args[]) throws IOException {
        ...
        try {
            ...
            // Creating a thread of the same, to run the receiving and sending process
            // parallel
        ...
        public void run() {
            ...
            try {
                ...
                while (serverThread.isAlive()) {
                    ...
                    switch (commands[0]) {
                        case "open":
                            // Creating the First copy of the Global and the Temporary States when the User
                            // register for the first time;
                            globalState = new customerInfo(commands[1], Double.parseDouble(commands[2]), commands[3],
                                    Integer.parseInt(commands[4]), Integer.parseInt(commands[5]));
                            tempState = new customerInfo(commands[1], Double.parseDouble(commands[2]), commands[3],
                                    Integer.parseInt(commands[4]), Integer.parseInt(commands[5]));
                        ...
                        case "withdrawal":
                            ...
                            System.out
                                    .println("The new Balance after withdrawal is $" + tempState.getBalance() + "\n\n");
                            break;
                        case "transfer":
                            // Initiating Transfer
                            ...
                            String tCommand = "transfer " + amt + " " + q + " " + tempState.getCustomerName() + " "
                                    + tempState.getFirst_label_sent()[labelUpdate];
                        ...
                        case "lost-transfer":
                            ...
                            System.out.println(
                                    "$" + lAmt + " fake Transaction has been initiated for client" + q + "\n\n");
                            o.println(tCommand);
                            o.flush();
                            o.close();
                            break;
                        ...
                        case "checkpoint":
                            ...
                            Thread.sleep(500);
                            // Counter of 'yes' recived by the Initiater , if matches the number of clients
                            // in chk_cohort then Make it Permanent.
                            ...
                            } // Incase there would have been a fake transaction then checkpointing is not
                              // approved. Hence, automatically triggers the Rollback procedure.
                            else {
                        ...
                        case "print cohort":
                            ...
                            break;
                        // Command to trigger Rollback algorithm manually, a permanent rollback is
                        // triggered on 'yes' received
                        case "rollback":
                    ...
                    // Putting the Server to Sleep, for waiting the other thread to finish reading
        ...
        public void run() {
            ...
            try {
                ...
                while ( /* !clientSocket.isClosed() && */ (input = in.readLine()) != null) {
                    ...
                    switch (command) {
                        case "transfer":
                            // Upon Receiving a Trasnfer the Sender Client has been added to the chkt_cohort
                            // list.
                            chkt_cohort.add(cohortList.get(line[3]));
                            int amt = Integer.parseInt(line[1]);
                            String p = line[2];
                            int updateLabel = Integer
                                    .parseInt(line[3].substring(line[3].length() - 1, line[3].length()));

                            // Check for a Fake Transaction
                            if (tempState.isWill_rollback()
                                    && (Integer.parseInt(line[4]) - tempState.getLast_label_recv()[updateLabel]) > 1
                                    && tempState.isResumeExecution()) {
                            ...
                            System.out.println(
                                    "$" + amt + " has been received. The total balance is $" + tempState.getBalance());
                            break;

                        case "lost-transfer":
                            // On receiving the lost-transaction the client is still added to the
                            // chkt_cohort, and the setWill_checkpoint is set to false.
                            chkt_cohort.add(cohortList.get(line[3]));
                        ...
                        case "prepare_to_rollback":
                            ...
                            if (tempState.isWill_rollback() && tempState.isResumeExecution()) {
                                ...
                                Socket socket = new Socket(cohortList.get(line[1]).getIPv4(),
                                        cohortList.get(line[1]).getPortB());
        ...
        public void run() {
            try {
                ...
                while (!(reply = in.readLine()).equals("disconnected")) {
                    ...
                    // Seperating the Cohort Information Responce from the Server to create a copy
                    // of cohort in clien
                    ...
                    if (!reply.isEmpty() && reply.charAt(0) >= 48 && reply.charAt(0) <= 57) {
                        ...
                        for (int i = 1; i <= nunOfCustomer; i++) {
                            ...
                            System.out.println(reply);
                            customerInfo newCustomer = new customerInfo(customer[0], Double.parseDouble(customer[1]),
                                    customer[2], Integer.parseInt(customer[3]), Integer.parseInt(customer[4]));
